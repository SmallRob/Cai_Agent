...
| **Gateway 全量** | Discord/Slack/Teams 已有 MVP；`gateway_production_summary_v1` 已提供本地生产状态摘要、readiness checklist 与 diagnostics，并内嵌 **`slash_deploy_check`**（**`gateway_slash_deploy_check_v1`**）；`gateway channel-monitor --json` / `GET /v1/gateway/channel-monitor` 已提供独立频道监控视图；`gateway slash-catalog --json` / `GET /v1/gateway/slash-catalog` 已提供 slash/command 能力目录；**`gateway slash-deploy-check --json` / `GET /v1/gateway/slash-deploy-check`** 已提供 Slack/Discord/Teams 部署自检清单；多工作区联邦执行链路仍为后续 Sprint | **24（后续）** |
