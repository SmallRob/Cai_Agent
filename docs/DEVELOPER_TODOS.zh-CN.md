...
> **当前无顺位开发单**。下一批候选见 [`NEXT_ACTIONS.zh-CN.md`](NEXT_ACTIONS.zh-CN.md) **后续队列**（如 **`HM-N12`** 真实云 POC 须在 **`CLOUD_RUNTIME_OOS`** §4.1 全满足后另独立项；**`CTX-COMPACT-N10`** 离线 fixtures 已落地）。
...
3. 上下文压缩：**`CTX-COMPACT-N09`/`N10`** 已落地；真模型长会话对比见 **`docs/qa/CTX_COMPACT_N10_REGRESSION.zh-CN.md`**（CI 外可选）。
