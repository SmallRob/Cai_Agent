...
| 最小只读 HTTP API（外部驱动 / CI） | Hermes | `Done` / `Next` | **已对齐 v0 + OpenAPI**：**`cai-agent api serve`**（**`CAI_API_PORT`** / **`CAI_API_TOKEN`**）；**`GET /healthz`**、**`/v1/status`**（**`api_status_v1`**）、**`/v1/doctor/summary`**、**`/v1/models`**（**`api_openai_models_v1`**）、**`/v1/models/summary`**、**`/v1/models/capabilities`**、非流式与 SSE **`POST /v1/chat/completions`**（**`api_openai_chat_completion_v1`** / **`api_openai_chat_completion_chunk_v1`**，复用 **`model_response_v1`**）、**`GET /v1/gateway/channel-monitor`**（**`gateway_channel_monitor_v1`**）、**`GET /v1/gateway/slash-catalog`**（**`gateway_slash_catalog_v1`**）、**`GET /v1/gateway/slash-deploy-check`**（**`gateway_slash_deploy_check_v1`**）、**`POST /v1/tasks/run-due`**（仅 dry_run）、**`GET /openapi.json`** 与 **`api openapi --json`**（**`api_openapi_v1`**）。OpenAPI 已暴露 ops interaction RBAC headers。**仍为 Next**：多 workspace 路由、更完整 operator 网关 |
