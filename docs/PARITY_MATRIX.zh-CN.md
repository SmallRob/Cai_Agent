...
| 可视化运营面板 | ECC | `Done` / `Next` | **已对齐**：CLI **`ops dashboard`**（JSON/text/html，**`--html-refresh-seconds`**）；**`ops serve`** HTTP（**`/v1/ops/healthz`** 探活 **`ops_liveness_v1`（无 Bearer）**、**`/v1/ops/dashboard`** / **`dashboard.html`** / **`dashboard/events`** / **`workspaces`**，**`CAI_OPS_API_TOKEN`** 可选）；**`/v1/ops/dashboard/interactions`** 收口为 **GET preview/audit**、**POST apply/preview/audit**；**`--role viewer|operator|admin`** 与 `X-CAI-Actor` / `X-CAI-Role` 为写路径提供 RBAC，审计行含 `role` / `workspace_scope`；**`ops_workspaces_v1`** 提供 allowlist 多 workspace 发现与 summary 聚合。**仍为 Next**：更完整 operator 路由等 |
