...
| `P2` 受控自动化与 operator 面 | 进行中：`GW-N02-D02`（联邦执行，设计阶段）；已完成：`GW-N02-D01`（**`gateway_slash_deploy_check_v1`** / **`slash_deploy_check`**）、`OPS-N02-D01`（**`/v1/ops/action-audit`**）、`BRW-N04`、`BRW-N05`、`OPS-RBAC-N01`、`OPS-MW-N01`、`GW-CHAN-N01`、`GW-SLASH-N01`、`OPS-N01-D01` | 详见 [`PLATFORM_SURFACES_SPRINT_PLAN.zh-CN.md`](PLATFORM_SURFACES_SPRINT_PLAN.zh-CN.md) |
