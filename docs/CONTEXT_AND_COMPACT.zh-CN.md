...
`CTX-COMPACT-N10` 起，仓库内提供 **离线长会话 fixture**（`cai-agent/tests/fixtures/context_compaction_regression/n10_case_*.json`）：pytest 对每条用例分别跑 **启发式** 与 **注入的 LLM 形摘要**（`evaluate_compaction_quality(..., summary_payload=...)`，不调用模型），用于固定回归基线；说明与可选真模型步骤见 [`docs/qa/CTX_COMPACT_N10_REGRESSION.zh-CN.md`](qa/CTX_COMPACT_N10_REGRESSION.zh-CN.md)。
