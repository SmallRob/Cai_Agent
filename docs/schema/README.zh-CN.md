...
- **`gateway slash-deploy-check`（GW-N02-D01）**：stdout **`gateway_slash_deploy_check_v1`**；按 Slack / Discord / Teams 输出 **`platforms[]`**（`id`、`configured`、`state`、`checks[]`；单条 check 含 `id`、`ok`（**`null`** 表示需在厂商控制台完成的步骤）、`severity`、`message`、可选 `next_step` / `doc_ref`），顶层 **`summary`** 含 `platforms_count` / `blocked` / `warn` / `ready` / `manual_check_count`。API **`GET /v1/gateway/slash-deploy-check`** 同源输出。
- **`gateway route-preview`（HM-N07-D03 / GW-N02-D02）**：stdout **`gateway_proxy_route_v1`**。默认 **`dry_run:true`**（仅预览，`route.decision`=`route_preview_only`）。**`dry_run:false`**（CLI **`--execute`** 或 API body）在同时满足 **`CAI_GATEWAY_FEDERATION_ROUTE_EXECUTE`**、目标 workspace 白名单（与源相同根目录始终允许；跨根须 **`CAI_GATEWAY_FEDERATION_ALLOWED_WORKSPACES`** 路径前缀列表，分隔符支持 `;`、`|`、换行）、**`target_profile_id`** 在目标侧有效 settings 中存在、可选 **`CAI_GATEWAY_FEDERATION_ROUTE_EXECUTE_TOKEN`** 与请求头 **`X-Cai-Federation-Execute-Token`** 一致时，将 **`gateway_federation_route_audit_v1`** 追加写入源工作区 **`.cai/gateway/federation-route-audit.jsonl`**，响应含 **`ok:true`**、**`route.decision`=`federation_route_committed`** 与 **`execution`**（**`gateway_federation_route_execution_v1`**）；不满足时 API 返回 **HTTP 403**，CLI **exit `2`**。不启动子进程、不调用 LLM。
...
- **动态 Web / HTTP 侧车**：只读 REST 与 MVP 分阶段说明见 **[`OPS_DYNAMIC_WEB_API.zh-CN.md`](../OPS_DYNAMIC_WEB_API.zh-CN.md)**；**Phase A**：**`--format html`** 可选 **`--html-refresh-seconds`**（**`meta refresh`**）；**Phase B** 由 **`cai-agent ops serve`**（**`cai_agent.ops_http_server`**）暴露 **`GET /v1/ops/healthz`**（**`ops_liveness_v1`**，**无 Bearer 鉴权**，探活用）、**`GET /v1/ops/dashboard`** / **`dashboard.html`** / **`dashboard/events`**（HTML 路由可选 query **`html_refresh_seconds`**、`live_mode=sse|poll`、`live_interval_seconds`；SSE 路由支持 `max_events`），载荷与上表同源。
- **多 workspace 发现（OPS-MW-N01）**：**`GET /v1/ops/workspaces`** 返回 **`ops_workspaces_v1`**，只枚举服务端 `--allow-workspace` allowlist 内的根目录；字段含 `workspaces_count`、`include_summary` 与 `workspaces[]`（`workspace`、`exists`、`allowed`、`dashboard_url`、`dashboard_html_url`、`interactions_url`、**`action_audit_url`**）。可选 query **`include_summary=1`** 时为每个存在的 workspace 内嵌同源 `ops_dashboard_v1.summary`，并复用 `observe_pattern`、`observe_limit`、`schedule_days`、`cost_session_limit` 参数。
- **受控写审计查询（OPS-N02-D01）**：**`GET /v1/ops/action-audit`** 返回 **`ops_action_audit_query_v1`**（**`variant`**=`single_workspace` 或 `aggregate`），**`items[]`** 为 **`ops_dashboard_action_audit_v1`** 行；可选 query **`workspace`**、**`limit`**、**`action`**、**`mode`**、**`ok`**、**`actor_prefix`**。OpenAPI 与 **`ops serve`** 启动横幅已登记。
...
| `sessions --compact-eval --json` | **`sessions_compact_eval_v1`**：汇总近期会话的压缩质量；每个会话嵌 **`context_compaction_eval_v1`**（schema: `cai-agent/src/cai_agent/schemas/context_compaction_eval_v1.schema.json`），含 `passed`、`score`、`checks`、`message_counts`、`estimated_tokens.reduction_ratio`、路径/工具/marker 保留率；可选 **`evaluation_variant`**（`heuristic` \| `llm_simulated`，离线回归）；任一失败 exit `2` | `--compact-keep-tail`、`--compact-summary-max-chars`、`--compact-required-marker`、`--compact-min-score` |
| `pytest test_context_compaction_regression_fixtures`（CTX-COMPACT-N10） | 固定 **`n10_case_*.json`** 长会话：对启发式与注入 LLM 形摘要分别跑 **`evaluate_compaction_quality`**，不调用模型；说明见 **`docs/qa/CTX_COMPACT_N10_REGRESSION.zh-CN.md`** | 同上 + fixture 目录 `cai-agent/tests/fixtures/context_compaction_regression/` |
...
| `GET /v1/ops/healthz` | **`ops_liveness_v1`**；ops 侧车探活；**不设 security**（与 **`ops serve`** 行为一致） |
| `GET /v1/ops/action-audit` | **`ops_action_audit_query_v1`**；合并或单根查询 **`ops_dashboard_action_audit_v1`** |
...
| `GET /v1/gateway/slash-deploy-check` | **`gateway_slash_deploy_check_v1`**；与 CLI `gateway slash-deploy-check --json` 同源，Slash / Teams 命令面部署自检清单 |
| `POST /v1/gateway/route-preview` | **`gateway_proxy_route_v1`**；默认 **`dry_run:true`**；**`dry_run:false`** 须联邦执行环境变量与白名单（见上 **`gateway route-preview`**）；可选请求头 **`X-Cai-Federation-Execute-Token`** |
