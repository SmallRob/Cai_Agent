...
| `GW-N02-D01` | `Done` | `HM-03` | Slash / Teams 命令面部署自检：**`gateway slash-deploy-check --json`**、**`GET /v1/gateway/slash-deploy-check`**（**`gateway_slash_deploy_check_v1`**）；**`gateway prod-status`** 内嵌 **`slash_deploy_check`** | `gateway_production.py`、`api_http_server.py`、`__main__.py` | `GW-SLASH-N01` | pytest + smoke |
| `GW-N02-D02` | `Done` | `HM-07` | 多工作区联邦：`route-preview` **`dry_run:false`** 在 **`CAI_GATEWAY_FEDERATION_ROUTE_EXECUTE`**、workspace 白名单与目标 profile 校验下写入 **`federation-route-audit.jsonl`**（**`gateway_federation_route_audit_v1`**）；可选 **`CAI_GATEWAY_FEDERATION_ROUTE_EXECUTE_TOKEN`** 与头 **`X-Cai-Federation-Execute-Token`**；不调用 LLM | `gateway_lifecycle.py`、`api_http_server.py`、`__main__.py` | `GW-N02-D01` | pytest + smoke |
| `HM-N12-D01` | `Done` | `HM-06` | 云后端：**不实现真实 Modal/Daytona 执行**（遵守 **[`CLOUD_RUNTIME_OOS.zh-CN.md`](CLOUD_RUNTIME_OOS.zh-CN.md)** §4.2）；§4.3 文档锚点 + **`runtime_backend_interface_v1`** 为 `modal`/`daytona` 增加 **`oos_policy`** / **`oos_doc`** / **`gate_ref`** | `runtime/registry.py`、`CLOUD_RUNTIME_OOS.zh-CN.md` | `HM-N11-D02` | pytest `test_runtime_local` + 文档走查 |
| `CTX-COMPACT-N09` | `Done` | `CTX-COMPACT` | 发往模型前的启发式隐私过滤（**非 NLP**、不读盘、不外传） | **`[privacy].filter`** / **`CAI_PRIVACY_FILTER`**（`off`/`light`/`strict`）；**`privacy_filter.py`**；**`llm_factory`** 适配器派发 + **`model_gateway.chat_response`**（OpenAI **`/v1/chat/completions`**、chat-smoke 等） | — | pytest **`test_privacy_filter`** + 全量 + smoke |
| `CTX-COMPACT-N10` | `Done` | `CTX-COMPACT` | 离线长会话压缩回归 fixtures（启发式 + 注入 LLM 形摘要） | **`n10_case_*.json`**（`tests/fixtures/context_compaction_regression/`）；**`evaluate_compaction_quality(..., summary_payload=)`**；**`context_compaction_eval_v1.evaluation_variant`**；**[`docs/qa/CTX_COMPACT_N10_REGRESSION.zh-CN.md`](docs/qa/CTX_COMPACT_N10_REGRESSION.zh-CN.md)** | `CTX-COMPACT-N09` | pytest **`test_context_compaction_regression_fixtures`** + 全量 + smoke |
