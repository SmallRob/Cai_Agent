...
| **B. Gateway 全量** | Discord/Slack/Teams MVP；`slash-catalog`、**`slash-deploy-check`**（`GW-N02-D01` Done）、`federation-summary`、`route-preview`（dry-run） | **联邦从预览到可选执行路径**（须显式开关与安全评审；**`GW-N02-D02`**） | 不默认铺 15+ 平台；一次收敛 1～2 个平台 |
...
5. **C1**：~~`HM-N12-D01`~~ **Done（契约层）**：**`CLOUD_RUNTIME_OOS`** §4.3 + **`runtime_backend_interface_v1`** 云 stub **OOS 元数据**；**真实** Modal/Daytona POC 仍须 §4.1 全满足后另独立项。
