...
| 多 workspace 发现（OPS-MW-N01） | `GET /v1/ops/workspaces` | 只枚举服务端 `--allow-workspace` allowlist；`include_summary=1` 时为每个 workspace 聚合 dashboard summary；每行含 **`action_audit_url`** |
| 操作审计查询（OPS-N02-D01） | `GET /v1/ops/action-audit` | **`ops_action_audit_query_v1`**：可选 **`workspace`** 或 allowlist 合并；过滤 **`action`** / **`mode`** / **`ok`** / **`actor_prefix`** / **`limit`** |
...
**用途**：负载均衡或编排探活；**不校验** **`CAI_OPS_API_TOKEN`**（与 **`GET /v1/ops/dashboard`** 等不同）。

**响应**

- **`200`**：JSON **`ops_liveness_v1`**，形如 **`{"ok": true, "service": "cai-agent-ops", "schema_version": "ops_liveness_v1"}`**。

### 3.0b `GET /v1/ops/action-audit`（**OPS-N02-D01**）

**用途**：只读查询 **`.cai/ops-dashboard-actions.jsonl`** 中的 **`ops_dashboard_action_audit_v1`** 行；支持**单工作区**或 **allowlist 全量合并**（跨工作区运营视图，边界仍为服务端 `--allow-workspace`）。

**Query**

| 参数 | 类型 | 默认 | 说明 |
|------|------|------|------|
| `workspace` | string | 可选 | 若省略：返回 **`variant=aggregate`**，合并所有 allowlist 根下最近事件（按 **`generated_at`** 降序截断 **`limit`**）；若指定：须为允许的工作区绝对路径，返回 **`variant=single_workspace`** |
| `limit` | int | `50` | 上限 **`500`** |
| `action` | string | 无 | 过滤 **`action`** 字段 |
| `mode` | string | 无 | 过滤 **`mode`**（大小写不敏感） |
| `ok` | bool | 无 | **`true`/`false`** 过滤成功/失败行 |
| `actor_prefix` | string | 无 | **`actor`** 前缀过滤 |

**响应**

- **`200`**：**`ops_action_audit_query_v1`**，含 **`variant`**、**`items[]`**（审计行）、**`filters`**、**`workspaces_scanned`**（aggregate 时）。
- **`400`/`403`**：非法 query、**`workspace`** 不在 allowlist。

**鉴权**：与其它 dashboard 路由相同（若配置了 **`CAI_OPS_API_TOKEN`** 则须 Bearer）。
